...
void Session::setListeningPort(int port)
{
    ...
    foreach (const QNetworkAddressEntry &entry, addresses) {
        ...
        if ((!listen_ipv6 && (protocol == QAbstractSocket::IPv6Protocol))
            ...
            continue;
        qDebug("Trying to listen on IP %s (%s)", qPrintable(ip_string), qPrintable(iface_name));
        m_nativeSession->listen_on(ports, ec, ip_string.toLatin1().constData(), libt::session::listen_no_system_port);
        if (!ec) {
            logger->addMessage(tr("qBittorrent is trying to listen on interface %1 port: %2", "e.g: qBittorrent is trying to listen on interface 192.168.0.1 port: TCP/6881").arg(ip_string).arg(QString::number(port)), Log::INFO);
            return;
        }
    }
